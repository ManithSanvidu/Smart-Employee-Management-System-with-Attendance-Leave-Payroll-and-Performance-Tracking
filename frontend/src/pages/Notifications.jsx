...
// ----------------------------------------------------
// 3D CARD TILT COMPONENT (Framer Motion 3D Tilt Effect)
// ----------------------------------------------------
const TiltCard = ({ children, className = "" }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    // Extremely subtle tilt (2 degrees max) for Stripe-like precision
...
// 3D Background removed in favor of static high-quality image layer

// ----------------------------------------------------
// INDIVIDUAL NOTIFICATION ITEM (Re-usable / Memoized)
// ----------------------------------------------------
const NotificationItem = memo(({ notification, actionId, handleMarkAsRead, handleDelete }) => {
  const isUnread = !notification.isRead;
  const busy = actionId === notification._id || actionId === `delete-${notification._id}`;
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start mb-1 gap-2">
            <h4 className={`text-[15px] font-semibold truncate transition-colors ${isUnread ? "text-zinc-900" : "text-zinc-600 group-hover:text-zinc-900"}`}>
              {notification.title}
            </h4>
...
// ----------------------------------------------------
const HeroSection = ({ unreadCount, emailStatus, setIsDrawerOpen, handleMarkAllAsRead, actionId }) => {
  return (
    <motion.div
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <img src={heroImg} alt="" className="w-full h-full object-cover object-center mix-blend-multiply opacity-80" />
      </div>
      ...
      <div className="relative p-8 md:p-10 flex flex-col md:flex-row md:items-end justify-between gap-8 z-10 min-h-[260px]">
        <div className="max-w-xl flex flex-col justify-between h-full">
          <div>
            ...
            <p className="text-[13.5px] text-zinc-500 leading-relaxed mb-8 max-w-[380px]">
              Stay up to date on attendance, leave, payroll and system alerts with a calm, enterprise grade overview.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-3 px-4 py-2.5 rounded-full bg-white border border-zinc-200 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
              ...
              <div className="flex flex-col justify-center">
                <span className="text-[12px] text-zinc-400 uppercase leading-none mb-1">Unread</span>
                <span className="text-[15px] font-semibold text-zinc-900 leading-none">{unreadCount}</span>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 self-start md:self-end mt-4 md:mt-0">
          {/* System Notification Center card */}
          <button
            type="button"
          ...
          >
            <span className="text-[12px] text-zinc-400 uppercase mb-1">System</span>
            <span className="text-[14px] font-medium text-zinc-900 leading-none">Notification center</span>
          </button>
...
// ----------------------------------------------------
// REPORTS / DOCUMENT HUB PANEL (Left 70%)
// ----------------------------------------------------
const ReportsPanel = ({ payrolls, selectedPayrollId, setSelectedPayrollId, pdfLoading, handlePdfDownload }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 15 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      {!hasAuthToken() && (
        <div className="mb-6 p-4 bg-zinc-50 border border-zinc-200 rounded-lg flex items-center justify-between">
          ...
          <Link to="/login" className="text-[14px] font-medium text-zinc-900 hover:text-zinc-600 transition-colors">
            Sign In &rarr;
          </Link>
        </div>
      )}
          ...
          <div className="h-full flex flex-col justify-between">
            <div className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-[15px] font-semibold text-zinc-900 leading-none">Monthly Payslip</h3>
                  <p className="text-[12px] text-zinc-400 mt-1">Payroll Service</p>
                </div>
              </div>
            ...
            <div className="flex gap-3 mt-auto pt-2">
              {payrolls.length > 0 && (
                <button
                  ...
                  onClick={() => handlePdfDownload("payslip", () => downloadPayslipPdf(payrolls.find(p => p._id === selectedPayrollId)))}
                ...
                >
                  <Download size={14} />
                  {pdfLoading === "payslip" ? "Downloading..." : "Download Payslip"}
                </button>
              )}
            </div>
          </div>
        ...
        <TiltCard>
          <div className="h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-[15px] font-semibold text-zinc-900 leading-none">Attendance Log</h3>
                    <p className="text-[12px] text-zinc-400 mt-1">Timesheet compilation</p>
                  </div>
                </div>
                <span className="text-[11.5px] font-medium bg-zinc-100 text-zinc-600 px-2 py-0.5 rounded-md">Admin</span>
              </div>
              <p className="text-[13.5px] text-zinc-500 leading-relaxed mb-6">
                Compile exhaustive monthly check-in logs, total operational hours, and exceptions.
              </p>
            </div>
            ...
            <button
              ...
              onClick={() => handlePdfDownload("attendance", downloadAttendanceReportPdf)}
            ...
            >
              <Download size={14} />
              {pdfLoading === "attendance" ? "Compiling..." : "Download Full PDF"}
            </button>
          </div>
        </TiltCard>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-[15px] font-semibold text-zinc-900 leading-none">Leave Balance</h3>
                    <p className="text-[12px] text-zinc-400 mt-1">Leave Analytics</p>
                  </div>
                </div>
                <span className="text-[11.5px] font-medium bg-zinc-100 text-zinc-600 px-2 py-0.5 rounded-md">Admin</span>
              </div>
              <p className="text-[13.5px] text-zinc-500 leading-relaxed mb-6">
                Detailed ledger summaries showing balance increments, pending approvals, and requests.
              </p>
            </div>
        ...
        <TiltCard>
          <div className="h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-[15px] font-semibold text-zinc-900 leading-none">Performance Index</h3>
                    <p className="text-[12px] text-zinc-400 mt-1">Appraisal Ledger</p>
                  </div>
                </div>
                <span className="text-[11.5px] font-medium bg-zinc-100 text-zinc-600 px-2 py-0.5 rounded-md">Admin</span>
              </div>
              <p className="text-[13.5px] text-zinc-500 leading-relaxed mb-6">
                Aggregate indices, goals metrics, peer-review progress, and standard benchmark scores.
              </p>
            </div>
            ...
            <button
              ...
              onClick={() => handlePdfDownload("performance", downloadPerformanceReportPdf)}
              className="w-full flex items-center justify-center gap-2 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 text-indigo-600 px-3 py-2 rounded-lg text-[14px] font-medium transition-all disabled:opacity-50"
            >
              <Download size={14} />
              {pdfLoading === "performance" ? "Compiling..." : "Download Full PDF"}
            </button>
          </div>
        </TiltCard>
...
// ----------------------------------------------------
// RECENT NOTIFICATIONS PANEL (Right 30%)
// ----------------------------------------------------
const RecentNotifications = ({ loading, recentNotifications, unreadCount, setIsDrawerOpen, actionId, handleMarkAsRead, handleDelete }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 15 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="flex-1 space-y-4">
        {loading ? (
          <div className="py-8 text-center text-[13.5px] text-zinc-500 leading-relaxed">Refreshing database...</div>
          ...
          <div className="py-8 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-[15px] font-semibold text-zinc-900">You're fully up-to-date!</p>
            <p className="text-[13.5px] text-zinc-500 leading-relaxed mt-1 max-w-[150px]">No new diagnostic alerts require immediate input.</p>
          </div>
        ) : (
...
// ----------------------------------------------------
// EMAIL STATUS WIDGET (Bottom Right)
// ----------------------------------------------------
const EmailStatusCard = ({ emailStatus, emailActionMsg, canTestEmail, handleSendTestEmail, emailLoading }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 15 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      {emailStatus && (
        <div className="space-y-3 bg-zinc-50/50 rounded-2xl p-4 border border-zinc-100/80 shadow-inner">
          <div className="flex justify-between items-center text-[13.5px] text-zinc-500 leading-relaxed">
            ...
            <span className="text-[14px] font-medium text-zinc-900">{emailStatus.emailEnabled ? "Active" : "Disabled"}</span>
          </div>
          <div className="flex justify-between items-center text-[13.5px] text-zinc-500 leading-relaxed">
            <span>SMTP Credentials</span>
            <span className="text-[14px] font-medium text-zinc-900">{emailStatus.smtpConfigured ? "Configured" : "Missing"}</span>
          </div>
          {emailStatus.verification?.message && (
            <div className="pt-3 mt-1 border-t border-zinc-200/50 flex justify-between items-center text-[12px] text-zinc-400">
              ...
              <span className={`font-medium ${emailStatus.verification.ok ? "text-emerald-600" : "text-amber-600 truncate max-w-[150px]"}`}>
                {emailStatus.verification.message}
              </span>
            </div>
          )}
      ...
      {canTestEmail && hasAuthToken() && (
        ...
        >
          ...
          {emailLoading === "test" ? "Sending test payload..." : "Send Test Email"}
        </button>
      )}
    </motion.div>
  );
...
const NotificationsDrawer = ({
  ...
  const filterTabs = ["All", "Unread", "Payroll", "Leave", "Attendance", "Performance"];
        ...
        <>
          ...
          >
            ...
            <div className="flex-shrink-0 px-6 py-5 border-b border-zinc-100 flex items-center justify-between bg-white">
              <div>
                <h2 className="text-[18px] font-semibold text-zinc-900">Notification Center</h2>
                <p className="text-[12px] text-zinc-400 mt-0.5">View and manage all your operational logs</p>
              </div>
              <button
                onClick={() => setIsDrawerOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-zinc-50/30">
              {loading ? (
                <div className="py-12 text-center text-[13.5px] text-zinc-500 leading-relaxed">Loading logs...</div>
                ...
                <div className="py-16 flex flex-col items-center justify-center text-center">
                  ...
                  <h3 className="text-[15px] font-semibold text-zinc-900 mb-1">No notifications found</h3>
                  <p className="text-[13.5px] text-zinc-500 max-w-[220px] leading-relaxed">
                    Try adjusting your filters or search query to find the specific reports.
                  </p>
                </div>
              ) : (
...
const Notifications = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      let message = err.response?.data?.message || err.message || "Failed to load notifications";
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      ...
      setEmailActionMsg(
        err.response?.data?.message || err.message || "Failed to send test email"
      );
    } finally {
      setEmailLoading("");
    }
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] p-6 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {error && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 bg-red-50 border border-red-100 text-red-600 rounded-xl text-xs flex items-start gap-3">
            <AlertCircle size={16} className="mt-0.5 flex-shrink-0" />
            <div>
              <p className="font-semibold">{error}</p>
              {!hasAuthToken() && (
                <Link to="/login" className="inline-block mt-1 font-semibold hover:underline">
                  Sign in to continue
                </Link>
              )}
            </div>
        ...
        {pdfMessage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`p-4 border rounded-xl text-xs flex items-start gap-3 ${
            pdfMessage.includes("successfully") ? "bg-emerald-50 border-emerald-100 text-emerald-700" : "bg-amber-50 border-amber-100 text-amber-700"
          }`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* RIGHT SIDE -> RECENT NOTIFICATIONS & EMAIL STATUS */}
